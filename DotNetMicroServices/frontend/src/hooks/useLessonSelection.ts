...
export const useLessonSelection = () => {
  const [selectedLessonForSlides, setSelectedLessonForSlides] =
    useState<any>(null);
  const [lastAutoSelectedLessonId, setLastAutoSelectedLessonId] = useState<
    string | null
  >(null);
  ...
  const isQuizLesson = selectedLessonForSlides
    ? (
        selectedLessonForSlides.lessonType ||
        selectedLessonForSlides.LessonType ||
        "standard"
      ).toLowerCase() === "quiz"
    : false;

  const isDiscussionLesson = selectedLessonForSlides
    ? (
        selectedLessonForSlides.lessonType ||
        selectedLessonForSlides.LessonType ||
        "standard"
  ...
  const selectedLessonId =
    selectedLessonForSlides?.id || selectedLessonForSlides?.Id || null;
