...
public class UserAccountGatewayService : IUserAccountGatewayService
{
    ...
    public async Task<ApiResponse<UserInfoDto>> UpdateUserStatusAsync(string id, string status)
    {
        try
        {
            ...
            var response = await _rabbitMQService.SendMessageAsync<ApiResponse<UserInfoDto>>(
                ...
                message);
            return response ?? ApiResponse<UserInfoDto>.ErrorResponse("Failed to update user status");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error calling UserAccountService to update user status");
            return ApiResponse<UserInfoDto>.ErrorResponse("An error occurred while updating user status");
        }
    }
}
