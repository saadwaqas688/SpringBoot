...
public class UserAccountService : IUserAccountService
{
    ...
    public async Task<UserAccount?> GetUserByEmailAsync(string email, CancellationToken cancellationToken = default)
    {
        ...
        return await _context.UserAccounts.Find(filter).FirstOrDefaultAsync(cancellationToken: cancellationToken);
    }

    public async Task<UserAccount?> GetUserByIdAsync(string id, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<UserAccount> CreateUserAsync(UserAccount user, CancellationToken cancellationToken = default)
    {
        ...
        await _context.UserAccounts.InsertOneAsync(user, cancellationToken: cancellationToken);
        return user;
    }

    public async Task<UserAccount?> UpdateUserAsync(string id, UserAccount existingUser, UpdateProfileDto dto, CancellationToken cancellationToken = default)
    {
        var filter = Builders<UserAccount>.Filter.Eq(u => u.Id, id);
        ...
        var updateResult = await _context.UserAccounts.UpdateOneAsync(filter, updateDefinition, cancellationToken: cancellationToken);
        ...
        // Return updated user
        var updatedUser = await _context.UserAccounts.Find(filter).FirstOrDefaultAsync(cancellationToken: cancellationToken);
    ...
    public async Task<PagedResponse<UserAccount>> GetAllUsersAsync(int page, int pageSize, string? searchTerm = null, CancellationToken cancellationToken = default)
    {
        ...
        var totalCount = await _context.UserAccounts.CountDocumentsAsync(filter, cancellationToken: cancellationToken);
    ...
    public async Task<UserAccount?> UpdateUserAdminAsync(string id, UpdateUserDto dto, CancellationToken cancellationToken = default)
    {
        var filter = Builders<UserAccount>.Filter.Eq(u => u.Id, id);
        var user = await _context.UserAccounts.Find(filter).FirstOrDefaultAsync(cancellationToken: cancellationToken);
    ...
    public async Task<bool> DeleteUserAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        {
            var filter = Builders<UserAccount>.Filter.Eq(u => u.Id, id);
            var result = await _context.UserAccounts.DeleteOneAsync(filter, cancellationToken: cancellationToken);
    ...
    public async Task<UserAccount?> UpdateUserStatusAsync(string id, string status, CancellationToken cancellationToken = default)
    {
        try
        {
            var filter = Builders<UserAccount>.Filter.Eq(u => u.Id, id);
            ...
            var result = await _context.UserAccounts.FindOneAndUpdateAsync(
                filter, 
                update, 
                new FindOneAndUpdateOptions<UserAccount> { ReturnDocument = ReturnDocument.After },
