...
public class UserAccountMessageHandler
{
    ...
    private async Task<object> HandleUpdateUserStatus(string messageJson)
    {
        try
        {
            ...
            {
                return ApiResponse<UserInfoDto>.ErrorResponse("Invalid update user status data");
            }

            if (request.Status != "active" && request.Status != "inactive")
            {
                return ApiResponse<UserInfoDto>.ErrorResponse("Status must be 'active' or 'inactive'");
            }
            ...
            return ApiResponse<UserInfoDto>.SuccessResponse(response, "User status updated successfully");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error in HandleUpdateUserStatus");
            return ApiResponse<UserInfoDto>.ErrorResponse($"Error updating user status: {ex.Message}");
        }
    }
