...
builder.Services.AddScoped<UserMessageHandler>();
...
rabbitMQService.StartListening(RabbitMQConstants.UserServiceQueue, async (messageJson, routingKey) =>
{
    // Create a scope for each message to resolve scoped services
    using var scope = app.Services.CreateScope();
    var messageHandler = scope.ServiceProvider.GetRequiredService<UserMessageHandler>();
